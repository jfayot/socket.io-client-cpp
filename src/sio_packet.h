...
namespace sio
{
  ...
  public:
    ...
    bool parse_msgpack(string const &payload_ptr); // return true if need to parse buffer.

    bool parse_buffer(string const &buf_payload);

    bool accept(string &payload_ptr, vector<shared_ptr<const string>> &buffers, bool use_msgpack = false); // return true if has binary buffers.
  ...
  public:
    ...
    void set_parser_default();

    void set_parser_msgpack();
