...
{
  /*************************public:*************************/
  client_impl::client_impl() : m_ping_interval(0),
                               m_ping_timeout(0),
                               m_network_thread(),
  ...
  void client_impl::set_parser_default()
  {
    m_packet_mgr.set_parser_default();
  }

  void client_impl::set_parser_msgpack()
  {
    m_packet_mgr.set_parser_msgpack();
  }
  ...
  void client_impl::on_client_disconnect(bool const &wasDisconnected)
  {
    if (m_connection_listener)
      m_connection_listener(wasDisconnected);
  }
  /*************************private:*************************/
  ...
  void client_impl::ping(const asio::error_code &ec)
  {
    ...
    m_packet_mgr.encode(p, [&](bool /*isBin*/, shared_ptr<const string> payload)
                        {
            lib::error_code ec;
            std::cout << "ping: " << *payload << std::endl;
